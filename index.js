...
// WORLD: everything you want to move for AVP locomotion goes here
...
// --- Orbit Controls (desktop only; disable during XR) ---
const orbit = new OrbitControls(camera, renderer.domElement);
orbit.target.set(0, 1.4, 0);
orbit.enableDamping = true;

// Locomotion (world movement)
const MOVE_SPEED = 1.4; // meters per second
let lastTime = null;
...
const handState = {
  left: { curls: {}, palm: "", thumbExtension: undefined },
  right: { curls: {}, palm: "", thumbExtension: undefined },
};
...
function initHandDebugMeshes() {
  ...
  ["left", "right"].forEach((handedness) => {
    const baseColor = handedness === "left" ? leftColor : rightColor;
    ...
    HAND_JOINT_NAMES.forEach((jointName) => {
      ...
      // IMPORTANT: keep these OUTSIDE worldRoot so they stay attached to user hands
      scene.add(mesh);
    ...
    // IMPORTANT: keep OUTSIDE worldRoot
    scene.add(palmMesh);
...
// --- Controller setup (kept in scene, not worldRoot) ---
...
function handleController(controller) {
  ...
  if (hits.length) {
    ...
    if (controller.userData.isSelecting) {
      hit.object.material.color.offsetHSL(0.005, 0, 0);
    }
  }
}

// ================== LEFT-WRIST SLIDER (WORLD-SPACE BUT USER-ANCHORED) ==================
...
// IMPORTANT: sliderRoot is in scene (NOT worldRoot) so it stays on your wrist while world moves
...
const valueToX = (v) =>
  THREE.MathUtils.mapLinear(v, SLIDER_MIN, SLIDER_MAX, -TRACK_LEN_M / 2, TRACK_LEN_M / 2);
...
const voltageLabel = new THREE.Mesh(new THREE.PlaneGeometry(0.3, 0.09), voltageMat);
...
function updateLeftHandSource(session) {
  ...
  for (const src of session.inputSources) {
    if (src.handedness === "left" && src.hand) {
      leftHandSource = src;
      break;
    }
  }
...
function bindSessionInputEvents(session) {
  ...
  session.addEventListener("selectend", (e) => {
    if (currentPinchSource === e.inputSource) currentPinchSource = null;
  });
}
...
function getLogicalInputs(session) {
  ...
  if (currentPinchSource) logical.pinch = currentPinchSource;
  return logical;
}
...
function updateHandDebug(frame, refSpace) {
  ...
  ["left", "right"].forEach((handedness) => {
    ...
    for (const fingerName in FINGERS) {
      ...
      // ✅ store thumb extension for BOTH hands
      if (fingerName === "thumb") {
    ...
    if (palmMesh) {
      ...
      if (indexPose && pinkyPose && middlePose) {
        ...
        if (handedness === "right") palmNormal.multiplyScalar(-1);
...
// --- Slider interaction (right hand pinch) ---
function updateSliderInteraction(frame) {
  ...
  const ht = rhs.hand;
  const tipIndex =
    ht.get?.("index-finger-tip") || (typeof XRHand !== "undefined" && XRHand.INDEX_PHALANX_TIP);
  const tipThumb =
    ht.get?.("thumb-tip") || (typeof XRHand !== "undefined" && XRHand.THUMB_PHALANX_TIP);
...
// ===================== Gesture locomotion (move worldRoot) =====================
...
function isMoveBackwardGesture() {
  const l = handState.left?.curls || {};
  return !!(l.index && l.pinky && l.ring && l.middle); // left: index+pinky+ring+middle curled
}

function applyGestureLocomotion(dt) {
  if (!dt || dt <= 0) return;

  const forward = isMoveForwardGesture();
  const backward = isMoveBackwardGesture();

  // avoid jitter if both/none
  ...
  if (forward) {
    // user forward => world back
    worldRoot.position.addScaledVector(_camFwd, -step);
    orbit.target.addScaledVector(_camFwd, -step);
  } else if (backward) {
    // user backward => world forward
    worldRoot.position.addScaledVector(_camFwd, +step);
    orbit.target.addScaledVector(_camFwd, +step);
  }
}

// ================== HUD (world-space plane attached to camera) ==================
const HUD_CFG = {
  WIDTH_PX: 1024,
  HEIGHT_PX: 512,
  PADDING: 18,
...
function labelFrom(src) {
  return (src.handedness || "none")[0].toUpperCase();
}
  ...
  for (const src of session.inputSources) {
    ...
    if (src.hand) {
      ...
      if (hs) {
        ...
        if (hs.thumbExtension !== undefined) {
...
function drawHud(header, bodyLines) {
  ...
  if (hiddenCount > 0) {
    hudCtx.fillText(`… +${hiddenCount} more`, P, H - P - HUD_CFG.LINE_H);
  }
...
renderer.xr.addEventListener("sessionstart", async () => {
  const session = renderer.xr.getSession();
  try {
    xrRefSpace_local = await session.requestReferenceSpace("local-floor");
  } catch {}
  ...
  // ✅ Disable OrbitControls in XR (prevents “UI stuck” feeling)
  orbit.enabled = false;
...
renderer.xr.addEventListener("sessionend", () => {
  ...
  // ✅ Re-enable OrbitControls on desktop
  orbit.enabled = true;
...
renderer.setAnimationLoop((t, frame) => {
  ...
  if (frame && xrRefSpace_local) {
    ...
    // ✅ locomotion AFTER curls updated
    applyGestureLocomotion(dt);
  }
  ...
  const bodyLines = xrSession && refSpace ? formatInputs(xrSession, frame, refSpace) : ["XR session not active."];
  drawHud(header, bodyLines);

  // Orbit only really matters on desktop (disabled in XR)
